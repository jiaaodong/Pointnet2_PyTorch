...
def model_fn_decorator(criterion):
    ...
    def model_fn(model, data, epoch=0, eval=False):
        with torch.set_grad_enabled(not eval):
            ...
            num_others = (labels==0).float().sum()
            num_ped = (labels==1).float().sum()
            num_biker = (labels==2).float().sum()
            num_car = (labels==3).float().sum()

            acc_TP_others = (True_labels==0).float().sum() / num_others

            # acc_T_ped_F_others = (False_labels_Pred[False_labels_GT==1]).numel() / num_ped
            acc_TP_pedestrian = (True_labels==1).float().sum() / num_ped
            ...
            acc_TP_biker = (True_labels==2).float().sum() / num_biker
            acc_TP_car = (True_labels==3).float().sum() / num_car
            # print( (True_labels==0).float().sum(), num_others,acc_TP_others)

        return ModelReturn(preds, loss, 
                                {
                                "Number of other targets":num_others.item(),
